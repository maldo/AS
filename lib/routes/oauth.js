...
server.grant(oauth2orize.grant.code(function(client, redirectURI, user, ares, done) {
	var code = rand.generateCryptoUid(16);

	db.authorizationCodes.save(code, client.id, redirectURI, user.uid, function(err) {
		if (err) { return done(err); }
		done(null, code);
	});
}));
...
server.exchange(oauth2orize.exchange.code(function(client, code, redirectURI, done) {
	db.authorizationCodes.find(code, function(err, authCode) {
		...
		var token = rand.generateCryptoUid(256);
		db.accessTokens.save(token, authCode.userUID, authCode.clientID, function(err) {
			if (err) { return done(err); }
			done(null, token);
		});
	});
